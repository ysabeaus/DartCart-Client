...
import {
  selectGetByUserStatus,
  getInvoicesByUser,
  selectInvoices,
  clearInvoices,
  resetState,
} from "../../common/slices/invoiceSlice";
  ...
  switch (result) {
    ...
    case "finished":
      for (let i = 0; i < invoices.length; ++i) {
        orders.push({
          cards: [],
          date: new Date(Number(invoices[i].orderPlaced)),
        });
        for (let j = 0; j < invoices[i].orderDetails.length; ++j)
          orders[i].cards.push(
            <OrderCard
              key={"Card: " + invoices[i].orderDetails[j].id}
              detail={invoices[i].orderDetails[j]}
            ></OrderCard>
          );
      }
      break;
              ...
              orders.map((order) => {
                return (
                  <>
                    ...
                    <div className="ProductCardContainer">{order.cards}</div>
                  </>
                );
              })
            ) : (
